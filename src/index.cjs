 ...
 */
function generateRoughnessMap(image, contrast = 1, brightness = 0, opacity = 1) {
 ...
 */
function generateNormalMap(image, intensity = 1) {
  const canvas = createCanvas(image.width, image.height);
  const ctx = canvas.getContext("2d");
 ...
 */
function generateMetalnessMap(image, threshold = 128, contrast = 1, brightness = 0, opacity = 1) {
  const canvas = createCanvas(image.width, image.height);
  const ctx = canvas.getContext("2d");
